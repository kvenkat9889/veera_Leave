...
async function fetchLeaveBalances(empId) {
  try {
    const response = await fetch(`http://98.80.67.100:3016/api/leave-balances/${empId}`, {
      method: 'GET',
    ...
    // Use form's empName if backend returns empty emp_name
    return {
      emp_name: data.emp_name || document.getElementById("empName").value.trim(),
      annual: data.annual,
      sick: data.sick,
      casual: data.casual
    };
    ...
    async function fetchLeaveHistory(empId) {
      try {
        const response = await fetch(`http://98.80.67.100:3016/api/leave-requests/${empId}`, {
          method: 'GET',
          headers: { 'Content-Type': 'application/json' }
        });
        if (!response.ok) {
...
async function submitLeaveRequest(request) {
  try {
    const response = await fetch('http://98.80.67.100:3016/api/leave-requests', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
